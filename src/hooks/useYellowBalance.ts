...
/**
 * Format Yellow Network balance (already in proper units, not wei)
 */
export function formatYellowBalance(
  balance: string | number | bigint,
  decimals: number = 4
): string {
  try {
    // Yellow Network balances are already in proper units, not wei
    const amount = typeof balance === "string"
      ? parseFloat(balance)
    ...
    console.error("[formatYellowBalance] Error:", error);
    return "0.00";
  }
}
